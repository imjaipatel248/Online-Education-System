        ...
        <div class="row" style="margin:0px;height:10000px" >
            ...
            <div class="right" style="background-color:#ddd;">
                ...
                try {
                    ...
                    if (!empty($_POST['id1'])) {
                        $query = $dbhandler->query('delete from admin_upload where id=' . $_POST['id1'] . ';');
                    }
                    $query = $dbhandler->query('select * from admin_upload where subject="dbms"');
                    while ($r = $query->fetch()) {
                        //echo specific attributes
                        $fnm = $r['filename'];
                        echo $r['id'];
                        echo "&nbsp;&nbsp;<img src='pdf.jpg' height=17 width=17/>&nbsp;&nbsp;<b>File:</b><a href='../upload/$fnm' target='_blank'>" . $fnm . '</a>&nbsp;&nbsp;&nbsp;<b>Description:</b>' . $r['description'];
                    ...
                    if (!empty($_POST['link']) || !empty($_POST['about'])) {
                        $query = $dbhandler->query('insert into youtube values(NULL,' . '"' . $_POST['link'] . '",' . '"dbms",' . '"' . $_POST['about'] . '");');
                    }


                    $query = $dbhandler->query('select * from youtube where branch="dbms"');
                    ...
                    if (isset($_COOKIE['ausername'])) {
                        ...
                        echo '<form action="sce.php" method="post" enctype="multipart/form-data">
                
                Upload File:<input type="file" name="myFile" id="myFile" required="true">
                <br><br>
                <textarea name="description" required="true">
                </textarea>
                <br> <input type="submit" value="Submit" id="jay" />
            </form>';
                        echo 'delete file:<form method="POST" action="">';
